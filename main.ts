...
export default class FirstLineIsTitle extends Plugin {
    ...
    outputAllSettings(): void {
        ...
        for (const key in this.settings) {
            const currentValue = (this.settings as any)[key];
    ...
    async onload(): Promise<void> {
        ...
        this.addSettingTab(new FirstLineIsTitleSettings(this.app, this as any));
    ...
    private setupDebugConsoleAPI(): void {
        ...
        if (!(window as any).DEBUG) {
            ...
            this._originalDebugEnable = (window as any).DEBUG.enable;
            this._originalDebugDisable = (window as any).DEBUG.disable;

            (window as any).DEBUG.enable = async (namespace?: string) => {
                if (namespace === 'first-line-is-title' || namespace === 'FLIT') {
                    await enableDebug();
            ...
            (window as any).DEBUG.disable = async (namespace?: string) => {
                if (namespace === 'first-line-is-title' || namespace === 'FLIT') {
                    await disableDebug();
    ...
    onunload() {
        ...
        // Cleanup console API
        delete (window as any).FLIT;

        // Restore or cleanup window.DEBUG
        if (this._createdDebugNamespace) {
            delete (window as any).DEBUG;
        } else if (this._originalDebugEnable || this._originalDebugDisable) {
            (window as any).DEBUG.enable = this._originalDebugEnable;
            (window as any).DEBUG.disable = this._originalDebugDisable;
        }
