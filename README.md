...
4. Extend the illegal character mapping functionality to vault search and the Quick switcher. For example, if `:` (regular colon) is replaced by `։` (replacement character) in file names, searching for `:` (regular) in vault search or the Quick switcher should automatically map to and match `։` (alt).
5. Add option to disable renaming file or folder in file and folder context menu.
6. Add option to rename file every time it gets opened (even if not focused).
7. Currently, if "Rename on focus" is toggled ON, the file gets renamed when it gets focused, not when it gets opened. E.g. if you open a file in a new tab without switching to it (if such option is enabled in Obsidian settings > Editor).
