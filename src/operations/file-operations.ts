...
export class FileOperations {
    ...
    async insertTitleOnCreation(file: TFile, initialContent?: string, templateContent?: string): Promise<boolean> {
        try {
            ...
            console.log(`[TITLE-REVERSAL] Convert replacement characters: ${this.settings.core.convertReplacementCharactersInTitle}`);
            if (this.settings.core.convertReplacementCharactersInTitle) {
                console.log(`[TITLE-REVERSAL] Starting reversal for: "${cleanTitle}"`);
                ...
                for (const settingKey of Object.values(charMap)) {
                    ...
                    if (replacement.enabled && replacement.replacement) {
                        ...
                        console.log(`[TITLE-REVERSAL] ${settingKey}: enabled=true, replacement="${replacement.replacement}"`);
                    }
                }
                ...
                if (count > 1) {
                    console.log(`[TITLE-REVERSAL] Skipping "${replacementChar}" → "${originalChar}" (duplicate, count=${count})`);
                    continue;
                }
                ...
                while (remaining.includes(replacementChar)) {
                    ...
                    if (before !== cleanTitle) {
                        console.log(`[TITLE-REVERSAL] Reversed "${replacementChar}" → "${originalChar}": "${before}" → "${cleanTitle}"`);
                    }
                }
                console.log(`[TITLE-REVERSAL] Final result: "${cleanTitle}"`);
            }
            ...
            } else if (this.settings.core.waitForTemplate) {
            ...
            if (contentAfterYaml !== '') {
                ...
                if (headingMatch) {
                    ...
                    for (const leaf of leaves) {
                        ...
                        if (view && view.file?.path === file.path && view.editor) {
                            ...
                            if (this.settings.core.moveCursorToFirstLine && this.settings.core.placeCursorAtLineEnd) {
                                const titleLength = titleWithHeading.length;
                                setTimeout(() => {
                                    if (view.editor) {
                                        view.editor.focus();
                                        view.editor.setCursor({ line: firstNonEmptyLineIndex, ch: titleLength });
                                        verboseLog(this.plugin, `[TITLE-INSERT] Positioned cursor at end of heading line ${firstNonEmptyLineIndex} (${titleLength} chars)`);
                                    }
                                }, 0);
                            }
                    ...
                    if (this.settings.core.moveCursorToFirstLine && this.settings.core.placeCursorAtLineEnd) {
                        // Check exclusions if disableCursorInExcludedNotes is ON
                        const isExcluded = this.settings.core.disableCursorInExcludedNotes
                            ? await this.isFileExcludedForCursorPositioning(file, currentContent)
                            : false;
            ...
            for (const leaf of leaves) {
                const view = leaf.view as MarkdownView;
                if (view && view.file?.path === file.path && view.editor) {
                    let titleLine: number;
                    if (yamlEndLine !== -1) {
                        titleLine = yamlEndLine + 1;
                        view.editor.replaceRange(finalTitle + '\n', { line: titleLine, ch: 0 });
                        verboseLog(this.plugin, `[TITLE-INSERT] Inserted title after frontmatter at line ${titleLine} via editor`);
                    } else {
                        titleLine = 0;
                        view.editor.replaceRange(finalTitle + '\n', { line: titleLine, ch: 0 });
    ...
    /**
     * Wait for view to be ready (has editor, file matches)
     * @param file - The file to wait for
     * @param maxRetries - Maximum retry attempts (default 10)
     * @returns MarkdownView if ready, null if not ready after maxRetries
     */
    async waitForViewReady(file: TFile, maxRetries = 10): Promise<MarkdownView | null> {
        for (let i = 0; i < maxRetries; i++) {
            const leaves = this.app.workspace.getLeavesOfType("markdown");
            for (const leaf of leaves) {
                const view = leaf.view as MarkdownView;
                if (view && view.file?.path === file.path && view.editor) {
                    verboseLog(this.plugin, `View ready for ${file.path} after ${i + 1} attempts`);
                    return view;
                }
            }
            if (i < maxRetries - 1) {
                verboseLog(this.plugin, `View not ready, retrying in ${TIMING.VIEW_READINESS_RETRY_DELAY_MS}ms...`);
                await new Promise(resolve => setTimeout(resolve, TIMING.VIEW_READINESS_RETRY_DELAY_MS));
            }
        }
        verboseLog(this.plugin, `View not ready for ${file.path} after ${maxRetries} attempts`);
        return null;
    }
    ...
    async handleCursorPositioning(file: TFile, usePlaceCursorAtLineEndSetting: boolean = true): Promise<void> {
        try {
            ...
            if (targetView && targetView.file?.path === file.path) {
                ...
                const shouldPlaceAtEnd = usePlaceCursorAtLineEndSetting && this.settings.core.moveCursorToFirstLine && this.settings.core.placeCursorAtLineEnd;
