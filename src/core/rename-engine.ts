...
export class RenameEngine {
    ...
    async processEditorChangeOptimal(editor: Editor, file: TFile): Promise<void> {
        ...
        try {
            ...
            // The actual processing will read from cache/file based on fileReadMethod
            ...
            // Pass editor content ONLY if fileReadMethod is 'Editor'
            // Otherwise, let processFileImmediate read from cache/file
            const providedContent = this.plugin.settings.core.fileReadMethod === 'Editor' ? currentContent : undefined;
            await this.processFileImmediate(file, providedContent, metadata);
        } catch (error) {
    ...
    async processFile(
        ...
        if (this.plugin.cacheManager?.isLocked(originalPath)) {
        ...
        this.plugin.cacheManager?.acquireLock(originalPath);
    ...
    private async processFileInternal(
        ...
        if (await hasDisablePropertyInFile(file, this.plugin.app, this.plugin.settings.exclusions.disableRenamingKey, this.plugin.settings.exclusions.disableRenamingValue)) {
            ...
            try {
                ...
                this.plugin.outputDebugFileContent(file, 'DISABLED', content);
            } catch (error) {
                console.error(`Error reading file content for debug output: ${file.path}`, error);
            }

            if (showNotices && !isBatchOperation) {
                verboseLog(this.plugin, `Showing notice: ${t('notifications.renamingDisabledForNote')}`);
                new Notice(t('notifications.renamingDisabledForNote'));
            }
        ...
        try {
            ...
            this.plugin.editorLifecycle.markFileAsRecentlyRenamed(newPath);
