        ...
        this.registerEvent(
            this.plugin.app.vault.on('modify', async (file) => {
                ...
                if (this.plugin.fileStateManager.hasPendingAliasRecheck(file.path)) {
                    ...
                    // Trigger alias update - pass isPendingAliasUpdate=true to bypass "file not open" check
                    // This is an intentional, explicit update we want to force after popover close
                    await this.plugin.aliasManager.updateAliasIfNeeded(file, undefined, undefined, false, undefined, undefined, true);
