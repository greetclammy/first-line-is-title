...
import { createMockFile, createMockApp, createTestSettings } from '../test/testUtils';
...
describe('RenameEngine', () => {
  ...
  describe('updateTitleRegionCacheKey', () => {
    it('should update cache key when file is renamed', () => {
      ...
      expect(plugin.fileStateManager.updateTitleRegionCacheKey).toHaveBeenCalledWith(
        oldPath,
        newPath
      );
    });
  ...
  describe('processEditorChangeOptimal', () => {
    ...
    it('should return early when file is locked', async () => {
      ...
      expect(plugin.cacheManager.markPendingAliasRecheck).toHaveBeenCalledWith(file.path);
    ...
    it('should not mark for recheck when editor is syncing', async () => {
      ...
      expect(plugin.cacheManager.markPendingAliasRecheck).not.toHaveBeenCalled();
    });
    ...
    it('should skip processing when only frontmatter changed', async () => {
      ...
      plugin.fileStateManager.getLastEditorContent = vi.fn().mockReturnValue(previousContent);
    ...
    it('should handle first editor event', async () => {
      plugin.fileStateManager.getLastEditorContent = vi.fn().mockReturnValue(null);
  ...
  describe('processFile', () => {
    beforeEach(() => {
      ...
      plugin.app.vault.read = vi.fn().mockResolvedValue('# Test Title\nContent');
    });
  ...
  describe('edge cases', () => {
    ...
    it('should handle null/undefined gracefully in stripFrontmatter', () => {
      expect(() => renameEngine.stripFrontmatterFromContent(null as any, file)).not.toThrow();
      expect(() => renameEngine.stripFrontmatterFromContent(undefined, file)).not.toThrow();
    });

    it('should handle very large content in stripFrontmatter', () => {
      const largeFrontmatter = '---\n' + 'x'.repeat(10000) + '\n---\nBody';

      const result = renameEngine.stripFrontmatterFromContent(largeFrontmatter, file);
