...
export class FileCreationCoordinator {
     ...
     */
    async determineActions(file: TFile, context: FileCreationContext): Promise<FileCreationActions> {
        this.decisionPath = [];
        ...
        if (this.hasExclusions()) {
            ...
            if (this.isFolderTemplatesEnabled()) {
                ...
                if (await this.folderTemplateMatches(file)) {
                    ...
                    return await this.handleTemplaterEvent(file, context, this.pathString());
                }
     ...
     */
    private async handleTemplaterEvent(file: TFile, context: FileCreationContext, pathSoFar: string): Promise<FileCreationActions> {
        // Node 12: Wait for Templater event (2000ms after ctime)
        const eventFired = await this.waitForTemplaterEvent(file, 2000, file.stat.ctime);
     ...
     */
    private async proceedToSettingsHub(file: TFile, context: FileCreationContext, pathSoFar: string): Promise<FileCreationActions> {
        const featuresEnabled = this.isFeatureEnabled();
        ...
        if (featuresEnabled === 'title') {
            ...
            if (this.isPlaceCursorAtEndEnabled()) {
                ...
                if (this.hasContentBelowYaml(context.initialContent)) {
                    ...
                    this.logDecision('18', 'N', 'Insert title + Move cursor + Place at end');
                    return {
                        shouldMoveCursor: true,
                        shouldInsertTitle: true,
                        placeCursorAtEnd: true,
    ...
    private async folderTemplateMatches(file: TFile): Promise<boolean> {
        ...
        while (folder) {
            const match = folderTemplates.find((ft: any) => ft.folder === folder!.path);
     ...
     */
    private async waitForTemplaterEvent(file: TFile, timeoutMs: number, ctime: number): Promise<boolean> {
        return new Promise((resolve) => {
            ...
            verboseLog(this.plugin, `Templater event: ${elapsed}ms elapsed since ctime, ${remainingTime}ms remaining for: ${file.path}`);

            // If already past timeout, return immediately
            if (remainingTime === 0) {
                verboseLog(this.plugin, `Templater event timeout already passed (${timeoutMs}ms after ctime) for: ${file.path}`);
                resolve(false);
                return;
            }
            ...
            const timeout = setTimeout(() => {
                if (!eventFired) {
                    verboseLog(this.plugin, `Templater event timeout (${timeoutMs}ms after ctime) for: ${file.path}`);
                    resolve(false);
                }
            }, remainingTime);
    ...
    private async templateHasExclusions(file: TFile): Promise<boolean> {
        try {
            ...
            const propertyStrategy = this.plugin.settings.exclusions.propertyScopeStrategy;
            ...
            if (excludedTags.length > 0) {
                ...
                const hasExcludedTag = allTags.some((tag: string) => excludedTags.includes(tag));
            ...
            if (excludedProps.length > 0 && frontmatter) {
                ...
                const hasWhitelistedProp = allPropKeys.some(key => whitelistedProps.includes(key));
     ...
     */
    private logDecision(nodeNumber: string, branch: string, outcome?: string): void {
        const entry = `${nodeNumber}${branch}`;
        this.decisionPath.push(entry);

        if (outcome) {
            verboseLog(this.plugin, `[FileCreation] Decision path: ${this.pathString()} → ${outcome}`);
        }
    }
