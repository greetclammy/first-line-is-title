...
export class IncludeExcludeTab extends SettingsTabBase {
    ...
    async render(): Promise<void> {
        ...
        const renderExcludedFolders = () => {
            ...
            this.plugin.settings.exclusions.excludedFolders.forEach((folder, index) => {
                const folderSetting = new Setting(folderContainer);
                let textInput: any;
                let removeButton: any;

                const updateButtonState = () => {
                    const isLastEmptyEntry = this.plugin.settings.exclusions.excludedFolders.length === 1 &&
                ...
                folderSetting.addText(text => {
                    ...
                    text.setPlaceholder(t('settings.exclusions.folders.placeholder'))
                        .setValue(folder)
                        .onChange(async (value) => {
                            this.plugin.settings.exclusions.excludedFolders[index] = value;
                            this.plugin.debugLog('excludedFolders', this.plugin.settings.exclusions.excludedFolders);
                            await this.plugin.saveSettings();
                            updateButtonState();
                        });
                    ...
                    try {
                        ...
                        const otherExclusions = this.plugin.settings.exclusions.excludedFolders.filter((_, i) => i !== index);
                        new FolderSuggest(this.plugin.app, text.inputEl, async (selectedPath: string) => {
                            this.plugin.settings.exclusions.excludedFolders[index] = selectedPath;
                            this.plugin.debugLog('excludedFolders', this.plugin.settings.exclusions.excludedFolders);
                            await this.plugin.saveSettings();
                            updateButtonState();
                        }, otherExclusions);
                ...
                .addExtraButton(button => {
                    ...
                    button.onClick(async () => {
                        ...
                        if (!isLastEmptyEntry) {
                            this.plugin.settings.exclusions.excludedFolders.splice(index, 1);
                            if (this.plugin.settings.exclusions.excludedFolders.length === 0) {
            ...
            const addButtonSetting = new Setting(folderContainer)
                .addButton(button => {
                    button.setButtonText(t('settings.exclusions.folders.addButton'))
                        .onClick(async () => {
                            const isBottomEntryEmpty = this.plugin.settings.exclusions.excludedFolders.length > 0 &&
                                                       this.plugin.settings.exclusions.excludedFolders[this.plugin.settings.exclusions.excludedFolders.length - 1].trim() === "";
        ...
        const tagUl = tagNotes.createEl('ul', { cls: "flit-margin-0 flit-padding-left-20" });
        ...
        const renderExcludedTags = () => {
            ...
            this.plugin.settings.exclusions.excludedTags.forEach((tag, index) => {
                ...
                const updateButtonState = () => {
                    const isLastEmptyEntry = this.plugin.settings.exclusions.excludedTags.length === 1 &&
                ...
                tagSetting.addText(text => {
                    ...
                    text.setPlaceholder(t('settings.exclusions.tags.placeholder'))
                        ...
                        .onChange(async (value) => {
                            ...
                            this.plugin.debugLog('excludedTags', this.plugin.settings.exclusions.excludedTags);
                            await this.plugin.saveSettings();
                            updateButtonState();
                        });
                    text.inputEl.classList.add('flit-width-100');
                    ...
                    try {
                        // Get all exclusions except the current one being edited
                        const otherExclusions = this.plugin.settings.exclusions.excludedTags.filter((_, i) => i !== index);
                        new TagSuggest(this.plugin.app, text.inputEl, async (selectedTag: string) => {
                            this.plugin.settings.exclusions.excludedTags[index] = selectedTag;
                            this.plugin.debugLog('excludedTags', this.plugin.settings.exclusions.excludedTags);
                            await this.plugin.saveSettings();
                            updateButtonState();
                        }, otherExclusions);
            ...
            const addTagButtonSetting = new Setting(tagContainer)
                .addButton(button => {
                    button.setButtonText(t('settings.exclusions.tags.addButton'))
                        .onClick(async () => {
                            const isBottomEntryEmpty = this.plugin.settings.exclusions.excludedTags.length > 0 &&
                                                       this.plugin.settings.exclusions.excludedTags[this.plugin.settings.exclusions.excludedTags.length - 1].trim() === "";
        ...
        const ul = propertyNotes.createEl('ul', { cls: "flit-margin-0 flit-padding-left-20" });
        ...
        const renderExcludedProperties = () => {
            ...
            if (this.plugin.settings.exclusions.excludedProperties.length === 0) {
                this.plugin.settings.exclusions.excludedProperties.push({ key: "", value: "" });
            }
            this.plugin.settings.exclusions.excludedProperties.forEach((property, index) => {
                ...
                const updateButtonState = () => {
                    const isLastEmptyEntry = this.plugin.settings.exclusions.excludedProperties.length === 1 &&
                ...
                keyInput.addEventListener('input', async (e: any) => {
                    this.plugin.settings.exclusions.excludedProperties[index].key = e.target.value;
                    this.plugin.debugLog('excludedProperties', this.plugin.settings.exclusions.excludedProperties);
                    await this.plugin.saveSettings();
                    updateButtonState();
                });

                valueInput.addEventListener('input', async (e: any) => {
                    this.plugin.settings.exclusions.excludedProperties[index].value = e.target.value;
                    this.plugin.debugLog('excludedProperties', this.plugin.settings.exclusions.excludedProperties);
                    await this.plugin.saveSettings();
                    updateButtonState();
                });
                ...
                propertySetting.addExtraButton(button => {
                    ...
                    button.onClick(async () => {
                        if (this.plugin.settings.exclusions.excludedProperties.length === 1) {
                            this.plugin.settings.exclusions.excludedProperties[0] = { key: "", value: "" };
                        } else {
                            this.plugin.settings.exclusions.excludedProperties.splice(index, 1);
                        }
            ...
            const addPropertyButtonSetting = new Setting(propertyContainer)
                .addButton(button => {
                    button.setButtonText(t('settings.exclusions.properties.addButton'))
                        .onClick(async () => {
                            const lastProperty = this.plugin.settings.exclusions.excludedProperties[this.plugin.settings.exclusions.excludedProperties.length - 1];
                            const isBottomEntryEmpty = this.plugin.settings.exclusions.excludedProperties.length > 0 &&
        ...
        const disableUl = propertyDisableNotes.createEl('ul', { cls: "flit-margin-0 flit-padding-left-20" });
        ...
        const propertyInputContainer = propertyControlWrapper.createDiv({ cls: "flit-property-container flit-display-flex flit-gap-10 flit-align-items-center" });
        ...
        keyInput.addEventListener('input', async (e) => {
            this.plugin.settings.exclusions.disableRenamingKey = (e.target as HTMLInputElement).value;
            await this.plugin.saveSettings();
        });

        valueInput.addEventListener('input', async (e) => {
            this.plugin.settings.exclusions.disableRenamingValue = (e.target as HTMLInputElement).value;
            await this.plugin.saveSettings();
        });

        propertyRestoreButton.addEventListener('click', async () => {
            this.plugin.settings.exclusions.disableRenamingKey = DEFAULT_SETTINGS.exclusions.disableRenamingKey;
            this.plugin.settings.exclusions.disableRenamingValue = DEFAULT_SETTINGS.exclusions.disableRenamingValue;
            keyInput.value = this.plugin.settings.exclusions.disableRenamingKey;
            valueInput.value = this.plugin.settings.exclusions.disableRenamingValue;
            await this.plugin.saveSettings();
        });
        const defaultTextContainer = this.containerEl.createEl("div", { cls: "setting-item-description flit-margin-top-5 flit-margin-bottom-20" });
