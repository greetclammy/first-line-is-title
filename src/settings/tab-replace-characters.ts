...
export class ForbiddenCharsTab extends SettingsTabBase {
    ...
    render(): void {
        const headerToggleSetting = new Setting(this.containerEl)
            ...
            .addToggle((toggle) => {
                toggle.setValue(this.plugin.settings.replaceCharacters.enableForbiddenCharReplacements)
                    .onChange(async (value) => {
                        this.plugin.settings.replaceCharacters.enableForbiddenCharReplacements = value;
                        ...
                        if (value && !this.plugin.settings.core.hasEnabledForbiddenChars) {
                            ...
                            allOSesKeys.forEach(key => {
                                this.plugin.settings.replaceCharacters.charReplacements[key as keyof typeof this.plugin.settings.replaceCharacters.charReplacements].enabled = true;
                            });
                            ...
                            if (currentOS === 'Windows' && !this.plugin.settings.core.hasEnabledWindowsAndroid) {
                        ...
                        if (windowsAndroidToggleComponent) {
                            ...
                            if (value) {
                                windowsAndroidToggleComponent.toggleEl.classList.remove('flit-state-disabled');
                                windowsAndroidToggleComponent.toggleEl.classList.add('flit-state-enabled');
                                windowsAndroidToggleComponent.toggleEl.tabIndex = 0;
                                windowsAndroidToggleComponent.toggleEl.removeAttribute('aria-disabled');
                            } else {
                                windowsAndroidToggleComponent.toggleEl.classList.remove('flit-state-enabled');
                                windowsAndroidToggleComponent.toggleEl.classList.add('flit-state-disabled');
                                windowsAndroidToggleComponent.toggleEl.tabIndex = -1;
                                windowsAndroidToggleComponent.toggleEl.setAttribute('aria-disabled', 'true');
                            }
                        }
                        (this.plugin as typeof this.plugin & { updateGeneralConditionalSettings?: () => Promise<void> }).updateGeneralConditionalSettings?.();
                    });
            });
        ...
        const primaryCharSettings: Array<{key: keyof typeof this.plugin.settings.replaceCharacters.charReplacements, name: string, char: string, description?: string}> = [
        ...
        const windowsAndroidChars: Array<{key: keyof typeof this.plugin.settings.replaceCharacters.charReplacements, name: string, char: string}> = [
        ...
        const updateCharacterSettings = () => {
            ...
            const allOSesNoteEl = charSettingsContainer.createEl("div", { cls: "setting-item-description flit-margin-top-15 flit-margin-bottom-15" });
            ...
            primaryCharSettings.forEach(setting => {
                ...
                toggleSetting.addToggle((toggle) => {
                    toggle.setValue(this.plugin.settings.core.hasEnabledForbiddenChars ? this.plugin.settings.replaceCharacters.charReplacements[key].enabled : false)
                        .onChange(async (value) => {
                            this.plugin.settings.replaceCharacters.charReplacements[key].enabled = value;
                            this.plugin.debugLog(`charReplacements.${String(key)}.enabled`, value);
                            await this.plugin.saveSettings();
                            updateRowAppearance();
                        });
                ...
                const updateRowAppearance = () => {
                    if (this.plugin.settings.replaceCharacters.charReplacements[key].enabled) {
                ...
                restoreButton.addEventListener('click', async () => {
                    this.plugin.settings.replaceCharacters.charReplacements[key].replacement = DEFAULT_SETTINGS.replaceCharacters.charReplacements[key].replacement;
                    textInput.value = DEFAULT_SETTINGS.replaceCharacters.charReplacements[key].replacement;
                    await this.plugin.saveSettings();
                });

                const textInput = inputContainer.createEl("input", { type: "text", cls: "flit-char-text-input flit-width-120" });
                textInput.placeholder = t('settings.replaceCharacters.emptyPlaceholder');
                textInput.value = this.plugin.settings.replaceCharacters.charReplacements[key].replacement;
                textInput.addEventListener('input', async (e) => {
                    this.plugin.settings.replaceCharacters.charReplacements[key].replacement = (e.target as HTMLInputElement).value;
                    this.plugin.debugLog(`charReplacements.${String(key)}.replacement`, this.plugin.settings.replaceCharacters.charReplacements[key].replacement);
                    await this.plugin.saveSettings();
                });
                ...
                trimLeftSetting.addToggle((toggle) => {
                    toggle.setValue(this.plugin.settings.core.hasEnabledForbiddenChars ? this.plugin.settings.replaceCharacters.charReplacements[key].trimLeft : false)
                        .onChange(async (value) => {
                            this.plugin.settings.replaceCharacters.charReplacements[key].trimLeft = value;
                            this.plugin.debugLog(`charReplacements.${String(key)}.trimLeft`, value);
                            await this.plugin.saveSettings();
                        });
                    toggle.toggleEl.classList.add('flit-margin-0');
                ...
                trimRightSetting.addToggle((toggle) => {
                    toggle.setValue(this.plugin.settings.core.hasEnabledForbiddenChars ? this.plugin.settings.replaceCharacters.charReplacements[key].trimRight : false)
                        .onChange(async (value) => {
                            this.plugin.settings.replaceCharacters.charReplacements[key].trimRight = value;
                            this.plugin.debugLog(`charReplacements.${String(key)}.trimRight`, value);
                            await this.plugin.saveSettings();
                        });
                    toggle.toggleEl.classList.add('flit-margin-0');
            ...
            const windowsAndroidHeaderSetting = new Setting(charSettingsContainer)
                ...
                .addToggle((toggle) => {
                    ...
                    toggle.setValue(this.plugin.settings.replaceCharacters.windowsAndroidEnabled)
                        .setDisabled(!this.plugin.settings.replaceCharacters.enableForbiddenCharReplacements)
                        .onChange(async (value) => {
                            this.plugin.settings.replaceCharacters.windowsAndroidEnabled = value;
                            this.plugin.debugLog('windowsAndroidEnabled', value);

                            // On first enable, turn on all 'Windows/Android' options
                            if (value && !this.plugin.settings.core.hasEnabledWindowsAndroid) {
                                windowsAndroidChars.forEach(setting => {
                                    const charKey = setting.key as keyof CharReplacements;
                                    this.plugin.settings.replaceCharacters.charReplacements[charKey].enabled = true;
                                });
                                this.plugin.settings.core.hasEnabledWindowsAndroid = true;
                                await this.plugin.saveSettings();
            ...
            windowsAndroidChars.forEach(setting => {
                ...
                const toggleSetting = new Setting(document.createElement('div'));
                toggleSetting.addToggle((toggle) => {
                    toggle.setValue((this.plugin.settings.core.hasEnabledForbiddenChars && this.plugin.settings.core.hasEnabledWindowsAndroid) ? this.plugin.settings.replaceCharacters.charReplacements[key].enabled : false)
                        .onChange(async (value) => {
                            this.plugin.settings.replaceCharacters.charReplacements[key].enabled = value;
                            this.plugin.debugLog(`charReplacements.${String(key)}.enabled`, value);
                            await this.plugin.saveSettings();
                ...
                const trimLeftSetting = new Setting(document.createElement('div'));
                trimLeftSetting.addToggle((toggle) => {
                    toggle.setValue((this.plugin.settings.core.hasEnabledForbiddenChars && this.plugin.settings.core.hasEnabledWindowsAndroid) ? this.plugin.settings.replaceCharacters.charReplacements[key].trimLeft : false)
                        .onChange(async (value) => {
                            this.plugin.settings.replaceCharacters.charReplacements[key].trimLeft = value;
                            this.plugin.debugLog(`charReplacements.${String(key)}.trimLeft`, value);
                            await this.plugin.saveSettings();
                ...
                const trimRightSetting = new Setting(document.createElement('div'));
                trimRightSetting.addToggle((toggle) => {
                    toggle.setValue((this.plugin.settings.core.hasEnabledForbiddenChars && this.plugin.settings.core.hasEnabledWindowsAndroid) ? this.plugin.settings.replaceCharacters.charReplacements[key].trimRight : false)
                        .onChange(async (value) => {
                            this.plugin.settings.replaceCharacters.charReplacements[key].trimRight = value;
                            this.plugin.debugLog(`charReplacements.${String(key)}.trimRight`, value);
                            await this.plugin.saveSettings();
