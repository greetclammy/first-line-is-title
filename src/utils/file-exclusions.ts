...
import { normalizeTag, parseInlineTagsFromText, stripFrontmatter, fileHasTargetTags } from './tag-utils';
...
export function shouldProcessFile(
    ...
    const shouldExcludeFromFolders = exclusionOverrides?.ignoreFolder ? false : applyStrategy(
        isInTargetFolders,
        settings.exclusions.excludedFolders.some(folder => folder.trim() !== ""),
    ...
    const shouldExcludeFromProperties = exclusionOverrides?.ignoreProperty ? false : applyStrategy(
        hasTargetProperties,
        settings.exclusions.excludedProperties.some(prop => prop.key.trim() !== ""),
    ...
    if (plugin?.settings.core.verboseLogging) {
        ...
        if (shouldExcludeFromFolders) reasons.push(`folder (${settings.exclusions.folderScopeStrategy})`);
        if (shouldExcludeFromTags) reasons.push(`tags (${settings.exclusions.tagScopeStrategy})`);
        if (shouldExcludeFromProperties) reasons.push(`properties (${settings.exclusions.propertyScopeStrategy})`);
