...
describe('content-reader', () => {
  ...
  describe('readFileContent', () => {
    ...
    it('should use provided editor when available', async () => {
      ...
      const result = await readFileContent(plugin, file, { providedEditor: editor });
    ...
    it('should search workspace for editor when searchWorkspace is true', async () => {
      ...
      const result = await readFileContent(plugin, file, { searchWorkspace: true });
    ...
    it('should use fresh read when fileStateManager indicates need', async () => {
      ...
      expect(result).toBe('Fresh content');
      expect(app.vault.read).toHaveBeenCalledWith(file);
      expect(plugin.fileStateManager.clearNeedsFreshRead).toHaveBeenCalledWith(file.path);
    });

    it('should throw error when file read fails', async () => {
      app.vault.cachedRead = vi.fn().mockRejectedValue(new Error('Read failed'));

      await expect(readFileContent(plugin, file)).rejects.toThrow('Failed to read file');
    });
    ...
    it('should prefer provided editor over workspace search', async () => {
      ...
      app.workspace.getLeavesOfType = vi.fn().mockReturnValue([
        { view: { file, editor: { getValue: vi.fn().mockReturnValue('Workspace') } } },
      ]);
  ...
  describe('workspace editor search', () => {
    ...
    it('should use active view editor as fallback', async () => {
      ...
      app.workspace.getLeavesOfType = vi.fn().mockReturnValue([]);
      app.workspace.getActiveViewOfType = vi.fn().mockReturnValue(mockActiveView);
