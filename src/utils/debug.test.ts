...
import { createMockApp, createTestSettings, createMockFile } from '../test/testUtils';
...
describe('DebugUtils', () => {
  ...
  describe('outputDebugFileContent', () => {
    ...
    it('should use N/A message when no editor content provided', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('N/A (no editor content available)');
    });
  ...
  describe('outputAllSettings', () => {
    it('should output all settings when verbose logging is enabled', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('SETTINGS: Complete configuration dump:');
    ...
    it('should output complete settings object', () => {
      ...
      expect(outputtedSettings.exclusions.excludedFolders).toContain('test-folder');
    });
  });
